...
function show(req, res, next) {
    fetch(
        `https://www.themealdb.com/api/json/v1/1/search.php?s=${req.params.recipe}`
    )
        .then((res) => res.json())
        .then((recipe) => {
...
function deleteRecipeFromList(req, res, next) {
    RecipeList.findById(req.params.recipeListId).then((recipeList) => {
        if (!recipeList.user.equals(req.user._id))
        ...
        return recipeList
            .save()
            .then(() =>
                res.redirect(`/recipe-lists/${req.params.recipeListId}`)
            )
            .catch(next);
    });
}
